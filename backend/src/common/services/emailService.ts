...
export const sendPasswordResetEmail = async (toEmail: string, otpCode: string): Promise<void> => {
  const config = getConfig();

  if (config.nodeEnv === "test") {
    logger.info(config, `[TEST EMAIL FALLBACK] Password reset OTP for ${toEmail}: ${otpCode}`);
    return;
  }
  ...
  if (!transporter) {
    logger.info(config, `[DEV EMAIL FALLBACK] Password reset OTP for ${toEmail}: ${otpCode}`);
    return;
  }
  ...
  const mailOptions: nodemailer.SendMailOptions = {
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e5e7eb; border-radius: 8px;">
        ...
        <p style="color: #4b5563; line-height: 1.5;">Chúng tôi nhận được yêu cầu đặt lại mật khẩu cho tài khoản của bạn tại cửa hàng trực tuyến. Vui lòng sử dụng mã xác thực bên dưới:</p>
        <div style="background-color: #f3f4f6; padding: 16px; border-radius: 6px; text-align: center; margin: 24px 0;">
          <span style="font-family: monospace; font-size: 32px; font-weight: bold; letter-spacing: 6px; color: #d97706;">${otpCode}</span>
        </div>
        <p style="color: #6b7280; font-size: 14px;">Mã này có hiệu lực trong <strong>10 phút</strong>. Nếu bạn không yêu cầu đặt lại mật khẩu, vui lòng bỏ qua email này.</p>
        <hr style="border: none; border-top: 1px solid #e5e7eb; margin: 24px 0;" />
        <p style="color: #9ca3af; font-size: 12px; text-align: center;">Đây là email tự động, vui lòng không phản hồi email này.</p>
      </div>
    `
