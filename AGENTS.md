...
## Frontend E-commerce Instructions
...
* Zustand for shared client state such as cart state
...
* Do not introduce Next.js, Redux, Medusa SDK, Tailwind, shadcn/ui, or another UI framework unless the task explicitly requires it.
* Do not fabricate products, prices, discounts, reviews, stock, shipping fees, or promotions.
* Use TanStack Query for server state.
* Use Zustand only for genuinely shared client state.
* Use local React state for component-local interactions.
* Use URL search parameters for search, filters, sorting, and pagination.

### Design direction

Build a clean production e-commerce storefront.

Avoid:

* Generic AI-generated layouts
* Unjustified purple gradients
* Glassmorphism
* Bento grids on every page
...
For storefront visual design:

* `$frontend-design`
* `$storefront-best-practices`

For browser and screenshot verification:
...
1. Run the frontend with `pnpm dev:frontend`.
2. Open the affected page using Playwright.
3. Test desktop at 1440 x 900.
4. Test mobile at 390 x 844.
5. Test the affected user flow.
6. Check browser console errors.
7. Check failed network requests.
8. Capture screenshots inside `screenshots/`.
9. Fix verified visual or functional problems.
10. Repeat the browser check after fixes.

A frontend task is not complete merely because it compiles.
